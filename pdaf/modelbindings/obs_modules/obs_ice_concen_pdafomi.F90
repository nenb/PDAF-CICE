...
MODULE obs_ice_concen_pdafomi
  ...
  REAL    :: rms_ice_concen=1      !< Observation error standard deviation (for constant errors)
  REAL    :: noise_amp = 1  ! Standard deviation for Gaussian noise in twin experiment

  ! One can declare further variables, e.g. for file names which can
  ! be use-included in init_pdaf() and initialized there.
  CHARACTER(len=110) :: file_ice_concen='/home/users/ys916780/PDAF_CICE/ic_files/iced.2008-01-01-00000.nc'  ! netcdf file holding observations
...
!!
  SUBROUTINE init_dim_obs_f_ice_concen(step, dim_obs_f)
    ...
    REAL, ALLOCATABLE :: ice_conc_field(:,:) ! Combination of aicen and vicen
...
! **********************************
    ...
    ! Read fractional ice area 'aicen'
    ALLOCATE(obs_field(nx_global, ny_global, ncat))
    ...
    s = s + 1
    stat(s) = NF90_GET_VAR(ncid_in, id_3dvar, obs_field, start=pos_nc, count=cnt_nc)
...
! ***********************************************************
    ...
    ! Compute ice concen observation field by summing over
    ! categories.
    ALLOCATE(ice_conc_field(nx_global,ny_global))
    ice_conc_field=0.0
    DO k = 1, ncat
       DO j = 1, ny_global
          DO i= 1, nx_global
             ice_conc_field(i,j)=ice_conc_field(i,j) + &
                  obs_field(i,j,k)
          END DO
       END DO
    END DO
    ...
    DO j = 1, ny_global
       DO i = 1, nx_global
          IF (ice_conc_field(i,j) .GE. 0.0) cnt = cnt + 1
       END DO
    END DO
    dim_obs_p = cnt
    dim_obs_f = cnt
    ...
    ALLOCATE(thisobs%id_obs_p(ncat, dim_obs_p))
    ...
    DO j = 1, ny_global
       DO i = 1, nx_global
          cnt0 = cnt0 + 1
          IF (ice_conc_field(i,j) .GE. 0) THEN
             cnt = cnt + 1
             DO k = 1, ncat
...
!!
  SUBROUTINE obs_op_f_ice_concen(dim_p, dim_obs_f, state_p, obsstate_f, offset_obs)

     USE PDAFomi_obs_op, &
         ONLY: PDAFomi_obs_op_f_gridavg
...
! ******************************************************
    ...
    IF (thisobs%doassim==1) THEN
       ...
       ! observation operator for averaging over categories
       CALL PDAFomi_obs_op_f_gridavg(dim_p, dim_obs_f, thisobs%dim_obs_p, &
            thisobs%dim_obs_f, 5, thisobs%id_obs_p, state_p, obsstate_f, offset_obs)
    END IF

  END SUBROUTINE obs_op_f_ice_concen
...
!! The routine is called by all filter processes.
!!
  SUBROUTINE deallocate_obs_ice_concen()
  ...
  END SUBROUTINE deallocate_obs_ice_concen
...
!! The routine is called by all filter processes.
!!
  SUBROUTINE init_obs_f_ice_concen(dim_obs_f, obsstate_f, offset_obs)
...
! ******************************************
  ...
  END SUBROUTINE init_obs_f_ice_concen
...
!! The routine is called by all filter processes.
!!
  SUBROUTINE init_obsvar_ice_concen(meanvar, cnt_obs)
...
! *****************************
  ...
  END SUBROUTINE init_obsvar_ice_concen
...
!! The routine is called by each filter process.
!!
  SUBROUTINE init_dim_obs_l_ice_concen(coords_l, lradius, dim_obs_l, offset_obs_l, offset_obs_f)
...
! ********************************************************************
  ...
  END SUBROUTINE init_dim_obs_l_ice_concen
...
!! The routine is called by all filter processes.
!!
  SUBROUTINE init_obs_l_ice_concen(dim_obs_l, obs_l)
...
! *******************************************
  ...
  END SUBROUTINE init_obs_l_ice_concen
...
!! The routine is called by all filter processes.
!!
  SUBROUTINE g2l_obs_ice_concen(dim_obs_l, dim_obs_f, obs_f, obs_l)
...
! *******************************************
  ...
  END SUBROUTINE g2l_obs_ice_concen
...
!! The routine is called by all filter processes.
!!
  SUBROUTINE prodRinvA_l_ice_concen(verbose, dim_obs_l, ncol, locweight, lradius, &
       sradius, A_l, C_l)
...
! ***********************
  ...
  END SUBROUTINE prodRinvA_l_ice_concen
...
!! The routine is called by all filter processes.
!!
  SUBROUTINE init_obsvar_l_ice_concen(meanvar_l, cnt_obs_l)
...
! ***********************************
  ...
  END SUBROUTINE init_obsvar_l_ice_concen
...
!! The routine is called by all filter processes.
!!
  SUBROUTINE prodRinvA_ice_concen(ncol, A_p, C_p)
...
! ***********************
  ...
  END SUBROUTINE prodRinvA_ice_concen
...
!! The routine is called by all filter processes.
!!
  SUBROUTINE add_obs_error_ice_concen(dim_obs, C_p)
...
! ***********************************************
  ...
  END SUBROUTINE add_obs_error_ice_concen
...
!! The routine is called by all filter processes.
!!
  SUBROUTINE init_obscovar_ice_concen(dim_obs, covar, isdiag)
...
! ***********************************************
  ...
  END SUBROUTINE init_obscovar_ice_concen
...
!! The routine is called by all filter processes.
!!
  SUBROUTINE likelihood_ice_concen(dim_obs, obs, resid, lhood)
...
! **************************
  ...
  END SUBROUTINE likelihood_ice_concen
...
!! assumed observation error statistics.
!!
  SUBROUTINE likelihood_l_ice_concen(verbose, dim_obs_l, obs_l, resid_l, &
...
! **************************
  ...
  END SUBROUTINE likelihood_l_ice_concen
...
!!
  SUBROUTINE localize_covar_ice_concen(verbose, dim, dim_obs, &
...
! **************************
  ...
  END SUBROUTINE localize_covar_ice_concen
