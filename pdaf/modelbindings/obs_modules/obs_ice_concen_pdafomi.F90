...
!!
  SUBROUTINE init_dim_obs_ice_concen(step, dim_obs)
...
! ********************
  ...
  END SUBROUTINE init_dim_obs_ice_concen
...
!!
  SUBROUTINE obs_op_ice_concen(dim_p, dim_obs, state_p, ostate)

    !USE PDAFomi, &
    !     ONLY: PDAFomi_obs_op_f_gridpoint
    USE PDAFomi_obs_op_extra, &
         ONLY: PDAFomi_obs_op_ice_concen
    USE ice_domain_size, &
         ONLY: ncat
...
! ******************************************************
    ...
    IF (thisobs%doassim==1) THEN
       ...
       ! observation operator for observed grid point values
       CALL PDAFomi_obs_op_ice_concen(thisobs, ncat, state_p, ostate)

    END IF

  END SUBROUTINE obs_op_ice_concen
...
!!
  SUBROUTINE init_dim_obs_l_ice_concen(domain_p, step, dim_obs, dim_obs_l)
