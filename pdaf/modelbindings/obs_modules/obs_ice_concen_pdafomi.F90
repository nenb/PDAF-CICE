...
! ***********************************************************
    ...
    ! Compute ice concen observation field
...
!!
  SUBROUTINE obs_op_f_ice_concen(dim_p, dim_obs_f, state_p, ostate_f, offset_obs)

    !USE PDAFomi, &
    !     ONLY: PDAFomi_obs_op_f_gridpoint
    USE PDAFomi_obs_op_extra, &
         ONLY: PDAFomi_obs_op_f_ice_concen
    USE ice_domain_size, &
         ONLY: ncat
...
! ******************************************************
    ...
    IF (thisobs%doassim==1) THEN
       ...
       ! observation operator for observed grid point values
       CALL PDAFomi_obs_op_f_ice_concen(thisobs, ncat, state_p, ostate_f, offset_obs)
...
!!
  SUBROUTINE init_dim_obs_l_ice_concen(domain_p, step, dim_obs_f, dim_obs_l, &
       off_obs_l, off_obs_f)
