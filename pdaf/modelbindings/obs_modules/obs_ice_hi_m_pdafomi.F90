...
MODULE obs_ice_hi_m_pdafomi
  ...
  REAL    :: rms_ice_hi_m=0.1      !< Observation error standard deviation (for constant errors)
...
! ***********************************************************
    ...
    DO j = 1, ny_global
       ...
       END DO
    END DO
    WRITE(*,*) "DEBUG HI_M FIELD: ", ice_hi_m_field(50,80)

    ! Count physically meaningful observations
    cnt = 0
    DO j = 1, ny_global
       DO i = 1, nx_global
          IF (ice_hi_m_field(i,j) >=0 .AND. ice_hi_m_field(i,j) <10) THEN 
             cnt = cnt + 1
          END IF
       END DO
    END DO
    ...
    DO j = 1, ny_global
       DO i = 1, nx_global
          cnt0 = cnt0 + 1
          IF (ice_hi_m_field(i,j) >=0 .AND. ice_hi_m_field(i,j) <=10.0) THEN
             cnt = cnt + 1
