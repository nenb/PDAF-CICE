!  SVN:$Id: ice_forcing.F90 1004 2015-06-16 20:13:46Z eclare $ 
!=======================================================================
!
...
! (restore_sst = T). SSS is not prognosed by CICE. 
      ...
      use ice_flux, only: sss, sst, qdp, uocn, vocn, Tf
...
! that filenames end with 'yyyy.dat' or 'yyyy.r' or 'yyyy.nc'.
      ...
      elseif (trim(atm_data_type) == 'cpom') then ! CPOM
         i = index(data_file,'.nc') - 5
         tmpname = data_file
         write(data_file,'(a,i4.4,a)') tmpname(1:i), yr, '.nc'
...
!                                          hm(i,j),   flw(i,j))
      ...
      if (calc_strair) then
        ...
        do i = ilo, ihi
           ...
           if (trim(atm_data_type) /= 'cpom') then
...
!=======================================================================
! CPOM prepared atmospheric forcing
!=======================================================================
!BOP
!
! !IROUTINE: cpom_files - construct filenames for cpom atmospheric data
!
! !INTERFACE:
!
...
!BOP
!
! !IROUTINE: cpom_data - read CPOM atmospheric data
!
! !INTERFACE:
!
...
! !USES:
!
      use ice_constants, only: c4, p5, secday, puny, &
          field_loc_center, field_type_scalar, field_type_vector
      use ice_flux, only: fsw, flw, frain, fsnow, Tair, uatm, vatm, Qa
!
! !INPUT/OUTPUT PARAMETERS:
!
!
...
! author: Elizabeth C. Hunke and William H. Lipscomb, LANL
      ...
      use ice_constants, only: c0, p5, c1000, depressT, &
          field_loc_center, field_type_scalar
      use ice_domain, only: nblocks
      use ice_flux, only: Tf, sss, sst, qdp, uocn, vocn, ss_tltx, ss_tlty
      ...
      if (my_task == master_task .and. istep == 1) then
         ...
         if (trim(uwat_data_type)=='clim') then
            write (nu_diag,*) ' '
            write (nu_diag,*) 'oce curr. data interpolated to timestep:'
            write (nu_diag,*) trim(uwat_file)
         endif
         if (trim(sss_data_type)=='clim') then
            write (nu_diag,*) ' '
