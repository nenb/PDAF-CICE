...
! (based on subroutine stress)
      ...
      use ice_constants, only: c0, c12, p027, p055, p111, p166, &
          p2, p222, p25, p333, p5, puny, pi
...
!          timer_tmp1, timer_tmp2, timer_tmp3
      ...
      real (kind=dbl_kind) :: &
        ...
        strp_tmp, strm_tmp, phi

      real (kind=dbl_kind) :: &
        alpharne, alpharnw, alpharsw, alpharse,     &
        alphasne, alphasnw, alphassw, alphasse

      phi = pi/c12 ! diamond shaped floe smaller angle (default phi = 30deg)
...
! tensor and for ksub=ndte it computes closing and sliding rate
      ...
      use ice_constants, only: c0, p05, p1, p5, c1, c2, c4, c12, &
          puny, pi, pih, pi2, piq
      ...
      real (kind=dbl_kind) :: &
         ...
         x, y, dx, dy, da, &
         invdx, invdy, invda,   &
         dtemp1, dtemp2, atempprime, a, invsin
...
! Can be set to 1 otherwise
         ...
         invsin = c1/sin(c2*phi) * invstressconviso 
...
! here Tan2gamma = 2 a12 / (a11 - a22) 
         ...
         if((ABS(a11 - a22) > puny).or.(ABS(a12) > puny)) then
           ...
           Angle_denom_gamma = c1/sqrt( ( a11 - a22 )*( a11 - a22) + &
                               c4*a12*a12 )
...
! here Tan2alpha = 2 dtemp12 / (dtemp11 - dtemp22) 
         ...
         if((ABS( dtemp11 - dtemp22) > puny).or. (ABS(dtemp12) > puny)) then
           ...
           Angle_denom_alpha = c1/sqrt( ( dtemp11 - dtemp22 )* &
...
! Tany = (Singamma*Cosgamma - Sinalpha*Cosgamma)/(Cos^2gamma -
! Sin^alpha)
...
!          invleng = c1/sqrt(Tany_1*Tany_1 + Tany_2*Tany_2) ! not sure
...
! Solves evolution equation for structure tensor (A19, A20) 
      ...
      use ice_constants, only: p001, p1, p2, p25, p5, c1, c10
      ...
      real (kind=dbl_kind) :: &
        mresult11, mresult12, &
        dteikth, p5kth, dtei_A

      real (kind=dbl_kind), parameter :: &
        kth  = p1*p2*p001             

      dtei_A = dtei/c10
      dteikth = c1 / (dtei_A + kth)
      p5kth = p5 * kth
...
! ne 
         ...
         a11_1(i,j) = (a11_1(i,j)*dtei_A  + p5kth - mresult11) * dteikth ! implicit
         a12_1(i,j) = (a12_1(i,j)*dtei_A  - mresult12) * dteikth ! implicit
...
! nw
         ...
         a11_2(i,j) = (a11_2(i,j)*dtei_A  + p5kth - mresult11) * dteikth ! implicit
         a12_2(i,j) = (a12_2(i,j)*dtei_A  - mresult12) * dteikth ! implicit
...
! sw
         ...
         a11_3(i,j) = (a11_3(i,j)*dtei_A  + p5kth - mresult11) * dteikth ! implicit
         a12_3(i,j) = (a12_3(i,j)*dtei_A  - mresult12) * dteikth ! implicit
...
! se
         ...
         a11_4(i,j) = (a11_4(i,j)*dtei_A  + p5kth - mresult11) * dteikth ! implicit
         a12_4(i,j) = (a12_4(i,j)*dtei_A  - mresult12) * dteikth ! implicit
...
! Eq. 7: Ffrac = -kf(A-S) or = 0 depending on sigma_1 and sigma_2
      ...
      use ice_constants, only: c0, c1, p001, p1, p5, c2, c3, c4, puny

      real (kind=dbl_kind), intent(in) :: &
         stressp, stressm, stress12, a11, a12
...
! add puny to the denominator to stop 1/0 errors, makes very little 
! error to the calculated angles < puny

       Q11Q11 = c1
       Q12Q12 = puny
       Q11Q12 = puny

       if((ABS( sigma11 - sigma22) > puny).or.(ABS(sigma12) > puny)) then

          Angle_denom = c1/sqrt( ( sigma11 - sigma22 )*( sigma11 - &
                        sigma22 ) + c4*sigma12*sigma12)

          Q11Q11 = p5 + ( sigma11 - sigma22 )*p5*Angle_denom   !Cos^2 
          Q12Q12 = p5 - ( sigma11 - sigma22 )*p5*Angle_denom   !Sin^2
