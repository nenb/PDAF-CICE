!  SVN:$Id: ice_therm_vertical.F90 1076 2015-11-11 04:41:22Z eclare $
!=========================================================================
!
...
!         C. M. Bitz, UW
      ...
      real (kind=dbl_kind), dimension (icells) :: &
         ...
         esnow_mlt   , & ! energy for snow loss, > 0    (J m-2)
         dhs_blow_tot, & ! change in snow thickness due to wind blow (total)
      ...
      real (kind=dbl_kind) :: &
         dhi         , & ! change in ice thickness
         dhs         , & ! change in snow thickness
         dhs_blow    , & ! change in snow thickness due to wind blow
         Ti          , & ! ice temperature
         Ts          , & ! snow temperature
...
! David: account for blowing snow based on Lecomte et al. 2015

        if (k.eq.1 .and. wind(i,j) > 3.5_dbl_kind) then

           dhs_blow  = -(wind(i,j) - 3.5_dbl_kind)/(1000000_dbl_kind*rhos)*dt
!          sigma_ITD = 1m  rho_max-rhos/rho_max = 0.14, gamma = 7x10-5
!          kgm-2
           if (hin(ij) < 0.6_dbl_kind) then
                dhs_blow = dhs_blow*2_dbl_kind
             elseif (hin(ij) < 1.4_dbl_kind) then
                dhs_blow = dhs_blow*1_dbl_kind
             elseif (hin(ij) < 2.4_dbl_kind) then
                dhs_blow = dhs_blow*0.5_dbl_kind
           else
                dhs_blow = c0
           end if
!          Only remove snow if thicker than 5cm!
           dhs_blow = min(c0,max(-dzs(ij,k)+0.05_dbl_kind,dhs_blow))
           esnow_mlt(ij) = -dhs_blow*zqsn(ij,k)
           dzs(ij,k) = dzs(ij,k) + dhs_blow
           dhs_blow_tot(ij) = dhs_blow
        endif
...
! David
         fhocnn(i,j) = fbot(i,j) &
                     + (esub(ij) + etop_mlt(ij) + ebot_mlt(ij) + &
                     esnow_mlt(ij))/dt
         freshn(i,j) = freshn(i,j) + dhs_blow_tot(ij) * rhos / dt
      enddo
